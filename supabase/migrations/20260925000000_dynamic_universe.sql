-- v88.0 DYNAMIC UNIVERSE (owner, 2026-09-24): the ledger no longer whitelists 40 names. Any well-formed Binance
-- USDT-perp base the runner selected (shared/universe.ts: listed + trading + crypto-only + liquid + valid data) may
-- be entered; stablecoin / non-crypto bases are refused here too. Everything else (profit gate required, 8 positions,
-- 25% per coin, 90% exposure, graded risk, paper 1x) unchanged from v87.0.
...
begin
 ...
 for x in select value from jsonb_array_elements(p_entries) loop
  ...
  -- v88.0: dynamic universe — any well-formed Binance USDT-perp base (the runner selects it from exchangeInfo, crypto only);
  -- stablecoin and known non-crypto bases are still refused here as a second line of defence (standing rule 2)
  if x->>'sym' !~ '^[A-Z0-9]{2,16}$' or x->>'sym' = any(array['USDC','FDUSD','TUSD','BUSD','DAI','USDP','USDE','USD1','PYUSD','XAU','XAG','PAXG','XAUT','BTCDOM','DEFI','TSLA','AAPL','NVDA','MSTR','AMZN','GOOGL','META','MSFT','SPY','QQQ']) or x->>'side' not in ('LONG','SHORT') then raise exception 'invalid scalp instrument'; end if;
 ...
 if p_minutes is not null then
  insert into team_meetings(decision,action,minutes) values(case when opens>0 then 'SCALP_OPEN' when paused then 'SCALP_PAUSED' else 'SCALP_HOLD' end,
    format('בוצעו ונשמרו %s פתיחות, %s סגירות. %s פוזיציות פתוחות; מזומן $%s. %s',opens,closes,countopen,round(cash,2),case when paused then 'כניסות מושהות: עצירה טכנית (hard halt).' else format('דמו אגרסיבי v88 על כל זוגות USDT הנזילים בבינאנס פיוצ׳רס, עד 8 פוזיציות, החזקה 5–240 דקות; כל כניסה עברה את שער הרווח (נטו ≥ 0.5 נק׳ בסיס); מכפיל סיכון %s.',round(rmult,2)) end),p_minutes);
 end if;
