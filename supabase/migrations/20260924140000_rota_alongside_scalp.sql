-- v83.0: (1) agent_stats.ev — scored snapshots per agent, for the cross-coin correction;
-- (2) scalp_commit_cycle: ROTA rows may share the book (position cap counts SCALP rows only,
--     a coin held by any sleeve is never doubled), planned holds up to 1440 minutes;
-- (3) rota_commit_cycle: the rotation sleeve's ledger — paper 1x, 20% per coin, basket
--     capped at share x 1.25 of equity, same lease check, sets rebalanced_at.
alter table public.agent_stats add column if not exists ev double precision not null default 0;
...
begin
 ...
 for x in select value from jsonb_array_elements(p_closes) loop
  ...
  funding:=case when t.strategy in ('SCALP','ROTA') then t.entry_price*t.size*0.0001*greatest(0,extract(epoch from now()-t.opened_at))/28800*(case when t.side='LONG' then 1 else -1 end) else 0 end;
 ...
 for x in select value from jsonb_array_elements(p_entries) loop
  ...
  n:=least((x->>'notional')::numeric,eq*0.5,greatest(0,eq*0.99-expo),greatest(0,cash/1.0005));
  ...
  values(x->>'sym',x->>'side',px,sz,n*0.0005,px*(1+(case when x->>'side'='LONG' then -1 else 1 end)*stop),px,px,'OPEN',true,'SCALP',1,n*stop,true,
   jsonb_build_object('stop_pct',stop,'hold_min',least(1440,greatest(1,coalesce((x->>'hold_min')::int,15))),'deadline',now()+make_interval(mins=>least(1440,greatest(1,coalesce((x->>'hold_min')::int,15)))),'max_deadline',now()+interval '1440 minutes','source',x->>'source','votes',x->'votes','experimental',true));
 ...
 if p_minutes is not null then
  insert into team_meetings(decision,action,minutes) values(case when opens>0 then 'SCALP_OPEN' when paused then 'SCALP_PAUSED' else 'SCALP_HOLD' end,
    format('בוצעו ונשמרו %s פתיחות, %s סגירות. %s פוזיציות פתוחות; מזומן $%s. %s',opens,closes,countopen,round(cash,2),case when paused then 'כניסות מושהות לפי מגבלת סיכון.' else 'מנוע דמו, החזקה 1–1440 דקות לפי אופק הסוכנים.' end),p_minutes);
 end if;
...
create or replace function public.rota_commit_cycle(p_lease timestamptz,p_closes jsonb,p_entries jsonb,p_marks jsonb,p_share numeric,p_note jsonb,p_rebalanced_at timestamptz default null)
returns jsonb language plpgsql security invoker set search_path = public,pg_temp as $$
declare
 s public.bot_state%rowtype; t public.bot_trades%rowtype; x jsonb; cfg jsonb;
 cash numeric; eq numeric; px numeric; n numeric; gross numeric; exitfee numeric; funding numeric; v_pnl numeric; rota_expo numeric;
 opens integer:=0; closes integer:=0; share numeric:=least(0.8,greatest(0.1,coalesce(p_share,0.5)));
begin
 select * into strict s from bot_state where id=1 for update;
 if p_lease is null or s.lock_until is distinct from p_lease or clock_timestamp()>p_lease then raise exception 'stale rota lease'; end if;
 if not s.active or not s.paper_mode then raise exception 'rota requires active paper account'; end if;
 if s.hard_halt_at is not null then raise exception 'hard halt is set'; end if;
 cash:=s.balance; cfg:=coalesce(s.bot_params,'{}');
 for x in select value from jsonb_array_elements(p_closes) loop
  select * into t from bot_trades where id=(x->>'id')::bigint and status='OPEN' and strategy='ROTA' for update;
  if not found then continue; end if;
  if t.paper_mode is not true or t.lev<>1 then raise exception 'non-paper or leveraged position'; end if;
  px:=(x->>'price')::numeric;
  if (x->>'quote_ts') is null or px is null or px<=0 or px='NaN'::numeric or abs(extract(epoch from clock_timestamp())*1000-(x->>'quote_ts')::numeric)>20000 then raise exception 'invalid close quote'; end if;
 ...
 for x in select value from jsonb_array_elements(p_entries) loop
  if exists(select 1 from bot_trades where status='OPEN' and sym=x->>'sym') then continue; end if;
  if not (x->>'sym'=any(array['BTC','ETH','SOL','BNB','XRP','DOGE','ADA','AVAX','LINK','DOT','LTC','BCH','NEAR','INJ','SUI','TRX','APT','ARB','OP','ATOM','FIL','UNI','AAVE','ICP','ALGO','SEI','WLD','TIA','RUNE','LDO','CRV','DYDX','GALA','SAND','AXS','IMX','ENA','PEPE','WIF','FET'])) or x->>'side' not in ('LONG','SHORT') then raise exception 'invalid rota instrument'; end if;
  px:=(x->>'price')::numeric;
  ...
  select coalesce(sum(entry_price*size),0) into rota_expo from bot_trades where status='OPEN' and strategy='ROTA';
  n:=least((x->>'notional')::numeric,eq*0.2,greatest(0,eq*share*1.25-rota_expo),greatest(0,cash/1.0005));
  if n<20 or n is null then continue; end if;
  insert into bot_trades(sym,side,entry_price,size,fee,trail_sl,hi,lo,status,paper_mode,strategy,lev,partial_done,scalp_meta)
  values(x->>'sym',x->>'side',px,n/px,n*0.0005,case when x->>'side'='LONG' then px*0.01 else px*100 end,case when x->>'side'='LONG' then px*100 else px end,case when x->>'side'='LONG' then px else px*0.01 end,'OPEN',true,'ROTA',1,true,
   jsonb_build_object('rota',true,'source',x->>'source','weight',(x->>'weight')::numeric,'hold_h',(x->>'hold_h')::numeric,'experimental',true));
  cash:=cash-n*1.0005; eq:=eq-n*0.0005; opens:=opens+1;
 end loop;
 cfg:=cfg||jsonb_build_object('rota_cycle',coalesce(p_note,'{}')||jsonb_build_object('ts',now(),'opened',opens,'closed',closes));
 update bot_state set balance=cash,paper_mode=true,updated_at=now(),rebalanced_at=coalesce(p_rebalanced_at,now()),bot_params=cfg where id=1;
 return jsonb_build_object('opened',opens,'closed',closes,'balance',cash,'equity',eq);
end $$;
revoke all on function public.rota_commit_cycle(timestamptz,jsonb,jsonb,jsonb,numeric,jsonb,timestamptz) from public,anon,authenticated;
grant execute on function public.rota_commit_cycle(timestamptz,jsonb,jsonb,jsonb,numeric,jsonb,timestamptz) to service_role;
