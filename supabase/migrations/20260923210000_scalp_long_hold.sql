-- v79.0: planned hold up to 240 minutes (agents are scored at 5/15/60/240 min net of costs;
-- a longer hold lets a move outgrow the fixed 16bps round trip). Hold caps 15->240 min and stop band 0.3-1% -> 0.3-4% (stop widens with sqrt(hold)).
-- v77.0 SCALP universe 8 -> the validated CRYPTO_40 (standing rule 2). Rest = v76.0.
...
begin
 ...
 for x in select value from jsonb_array_elements(p_entries) loop
  ...
  values(x->>'sym',x->>'side',px,sz,n*0.0005,px*(1+(case when x->>'side'='LONG' then -1 else 1 end)*stop),px,px,'OPEN',true,'SCALP',1,n*stop,true,
   jsonb_build_object('stop_pct',stop,'hold_min',least(240,greatest(1,coalesce((x->>'hold_min')::int,15))),'deadline',now()+make_interval(mins=>least(240,greatest(1,coalesce((x->>'hold_min')::int,15)))),'max_deadline',now()+interval '240 minutes','source',x->>'source','votes',x->'votes','experimental',true));
 ...
 if p_minutes is not null then
  insert into team_meetings(decision,action,minutes) values(case when opens>0 then 'SCALP_OPEN' when paused then 'SCALP_PAUSED' else 'SCALP_HOLD' end,
    format('בוצעו ונשמרו %s פתיחות, %s סגירות. %s פוזיציות פתוחות; מזומן $%s. %s',opens,closes,countopen,round(cash,2),case when paused then 'כניסות מושהות לפי מגבלת סיכון.' else 'מנוע דמו, החזקה 1–240 דקות לפי אופק הסוכנים.' end),p_minutes);
 end if;
