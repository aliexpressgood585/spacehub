...
begin
 ...
 if s.lock_until is distinct from p_lease or clock_timestamp()>p_lease then raise exception 'stale scalp lease'; end if;
 ...
 for x in select value from jsonb_array_elements(p_closes) loop
  ...
  px:=(x->>'price')::numeric;
  if px is null or px<=0 or px='NaN'::numeric or abs(extract(epoch from clock_timestamp())*1000-(x->>'quote_ts')::numeric)>20000 then raise exception 'invalid close quote'; end if;
 ...
 for x in select value from jsonb_array_elements(p_entries) loop
  if p_minutes is null or paused or countopen>=4 then continue; end if;
  ...
  if not (x->>'sym'=any(array['BTCUSDT','ETHUSDT','SOLUSDT','XRPUSDT','DOGEUSDT','ADAUSDT','LINKUSDT','AVAXUSDT'])) or x->>'side' not in ('LONG','SHORT') then raise exception 'invalid scalp instrument'; end if;
  px:=(x->>'price')::numeric; stop:=(x->>'stop_pct')::numeric;
  if px is null or px<=0 or px='NaN'::numeric or stop not between 0.003 and 0.01 or abs(extract(epoch from clock_timestamp())*1000-(x->>'quote_ts')::numeric)>20000 then raise exception 'invalid entry quote'; end if;
  ...
  values(x->>'sym',x->>'side',px,sz,n*0.0005,px*(1+(case when x->>'side'='LONG' then -1 else 1 end)*stop),px,px,'OPEN',true,'SCALP',1,n*stop,true,
   jsonb_build_object('stop_pct',stop,'deadline',now()+interval '15 minutes','source',x->>'source','votes',x->'votes','experimental',true));
