...
export async function runScalp(db:any,state:any,lease:string,paper:boolean,rotaShare:number=0) {  // v83.0: rotaShare>0 = the ROTA sleeve shares this book
  ...
  for(const t of open) {
    ...
    if(t.strategy==='ROTA'||t.strategy==='BRKV'||t.strategy==='LAB'){retained.push(t);exposure+=notional;continue}   // v93.0: BRKV is foreign too; v94.0: LAB too
  ...
  const eligible=failures.length<=Math.floor(Math.max(1,SCAN.length)*0.2)&&open.every((t:any)=>data.has(String(t.sym)))&&!retained.some(t=>!['SCALP','ROTA','BRKV','LAB'].includes(t.strategy))&&!state.hard_halt_at&&!params.scalp_paused
