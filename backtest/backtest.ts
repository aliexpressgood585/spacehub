...
// ─────────────────────────────────────────────────────────────
// v51bt: SIZING TILTS round 2 — same trades, smarter capital:
//  A) volatility compression ("squeeze"): breakout from a historically narrow
//     Donchian channel (width percentile vs trailing 90 bars) → bigger size.
//  B) weekend: is a Sat/Sun UTC breakout worth more or less than weekday?
// Deploy bar (same as v49bt): beats baseline in ALL 6 windows AND ≥+0.004R total.
function runV51bt() {
  ...
  const wSpan=(tmax-tmin)/NW
  const winOf=(t:number)=>Math.min(NW-1,Math.max(0,Math.floor((t-tmin)/wSpan)))
  console.log(`  loaded ${Object.keys(d4).length} coins`)
  ...
  interface Tr { win:number; net:number; compPct:number|null; weekend:boolean }
  const trades:Tr[]=[]
  for (const c of Object.keys(d4)) {
    if (!CORE40.has(c)) continue
    const arr=d4[c]; let last=-999
    // rolling 25-bar channel width series
    const width:number[]=new Array(arr.length).fill(NaN)
    for (let i=25;i<arr.length;i++) {
      let hi=-Infinity, lo=Infinity
      for (let k=i-25;k<i;k++) { if (arr[k].high>hi) hi=arr[k].high; if (arr[k].low<lo) lo=arr[k].low }
      width[i]=(hi-lo)/arr[i].close
    }
    for (let i=100;i<arr.length-1;i++) {
      const price=arr[i].close
      ...
      // compression percentile of current width vs trailing 90 bars
      let compPct:number|null=null
      if (i>=115 && Number.isFinite(width[i])) {
        let below=0, n=0
        for (let k=i-90;k<i;k++) { if (Number.isFinite(width[k])) { n++; if (width[k]<width[i]) below++ } }
        if (n>=60) compPct=below/n
      }
      const dow=new Date(arr[i].t).getUTCDay()
      trades.push({win:winOf(arr[i].t), net, compPct, weekend: dow===0||dow===6})
    }
  }
  console.log(`  ${trades.length} trades; compPct on ${trades.filter(x=>x.compPct!==null).length}; weekend share ${(100*trades.filter(x=>x.weekend).length/trades.length).toFixed(0)}%`)
  ...
  const evalTilt=(name:string, wOf:(tr:Tr)=>number)=>{
    ...
    console.log(`  ${name.padEnd(34)} avg ${(tT>=0?'+':'')+tT.toFixed(4)}R vs base ${(bT>=0?'+':'')+bT.toFixed(4)}R | ${tA.map((x,i)=>(((x>=0?'+':'')+x.toFixed(3))+(better[i]?'▲':'▽')).padStart(8)).join(' ')} ${okAll&&okBetter?'✅':okAll?'(pos, not uniformly better)':''}`)
  }
  console.log(`\n── tilt evaluation (▲ = beats baseline in that window) ──`)
  evalTilt('A squeeze 1.25/0.75 @30/70pct', tr=>tr.compPct===null?1:(tr.compPct<0.3?1.25:tr.compPct>0.7?0.75:1))
  evalTilt('A squeeze 1.5/0.5 @30/70pct',   tr=>tr.compPct===null?1:(tr.compPct<0.3?1.5:tr.compPct>0.7?0.5:1))
  evalTilt('A squeeze CONTRA (wide=big)',   tr=>tr.compPct===null?1:(tr.compPct>0.7?1.25:tr.compPct<0.3?0.75:1))
  evalTilt('B weekend BOOST 1.25',          tr=>tr.weekend?1.25:1)
  evalTilt('B weekend CUT 0.75',            tr=>tr.weekend?0.75:1)
  console.log(`\n✅ = positive AND better than baseline in all ${NW} windows. Deploy bar: ✅ plus ≥+0.004R total.`)
}
...
// ─────────────────────────────────────────────────────────────
// v70bt: linear-regression channel breakout vs Donchian — the last genuinely
//  different breakout DEFINITION. Fit LSMA over 15 bars, channel = LSMA ± k·σ;
//  breakout = close exits the channel (vs Donchian high/low). Same ADX/SL/ladder.
//  Deploy bar: beats Donchian-15 (696R baseline) in all 6 windows.
function runV70bt() {
  const TK=0.0005, MK=0.0002, NW=6, BAR4=14400000
  const toTf=(a:Bar[],ms:number):Bar[]=>{
    const out:Bar[]=[]; let cur:Bar|null=null; let bucket=-1
  ...
  // LSMA endpoint + residual sigma over last W closes ending at index i-1
  const regChannel=(arr:Bar[],i:number,W:number)=>{
    let sx=0,sy=0,sxx=0,sxy=0
    for(let k=0;k<W;k++){const x=k,y=arr[i-W+k].close;sx+=x;sy+=y;sxx+=x*x;sxy+=x*y}
    const den=W*sxx-sx*sx;if(Math.abs(den)<1e-12)return null
    const b=(W*sxy-sx*sy)/den,a=(sy-b*sx)/W
    const end=a+b*(W-1)  // regression value at the last bar of the window
    let s2=0;for(let k=0;k<W;k++){const pred=a+b*k;const r=arr[i-W+k].close-pred;s2+=r*r}
    return {mid:end,sd:Math.sqrt(s2/W)}}

  const scan=(mode:'donch'|'reg',K:number)=>{
    const ws:{n:number,w:number,sum:number}[]=Array.from({length:NW},()=>({n:0,w:0,sum:0}))
    for(const c of Object.keys(d4)){
      if(!CORE40.has(c))continue
      const arr=d4[c];let last=-999
      for(let i=100;i<arr.length-1;i++){
        ...
        if(mode==='donch'){
          const prior=arr.slice(i-15,i);let hi=-Infinity,lo=Infinity
          for(const b of prior){if(b.high>hi)hi=b.high;if(b.low<lo)lo=b.low}
          side=price>hi?'LONG':price<lo?'SHORT':null
        }else{
          const ch=regChannel(arr,i,15);if(!ch||ch.sd<=0)continue
          side=price>ch.mid+K*ch.sd?'LONG':price<ch.mid-K*ch.sd?'SHORT':null
        }
        if(!side)continue
        const win=arr.slice(Math.max(0,i-99),i+1)
        const adx=calcADX(win.slice(-60));if(adx<=22)continue
        ...
        const res=ladder(arr,i,price,side,slDist,atr,i+96)
        const net=res.r-(TK+res.tpFrac*MK+(1-res.tpFrac)*TK)/slPct
        const w=ws[winOf(arr[i].t)];w.n++;w.sum+=net;if(net>0)w.w++}
    }
    const tot=ws.reduce((a,x)=>({n:a.n+x.n,w:a.w+x.w,sum:a.sum+x.sum}),{n:0,w:0,sum:0})
    const wA=ws.map(x=>x.n?x.sum/x.n:0)
    return {n:tot.n,wr:100*tot.w/tot.n,avg:tot.sum/tot.n,tot:tot.sum,wA,allPos:wA.every(x=>x>0)}}

  console.log(`\n── REGRESSION-CHANNEL breakout vs DONCHIAN-15 ──`)
  const fmt=(r:any,nm:string,base?:number)=>console.log(`  ${nm.padEnd(24)} n=${String(r.n).padStart(5)} WR=${r.wr.toFixed(1)}% avg ${(r.avg>=0?'+':'')}${r.avg.toFixed(4)}R totR ${r.tot.toFixed(0)} ${r.allPos?'✅6w':''}${base!==undefined?(r.tot>base?' >base':' ≤base'):''} | ${r.wA.map((x:number)=>((x>=0?'+':'')+x.toFixed(3)).padStart(7)).join(' ')}`)
  const d=scan('donch',0)
  fmt(d,'Donchian-15 (live)')
  fmt(scan('reg',1.5),'reg-channel k=1.5',d.tot)
  fmt(scan('reg',2.0),'reg-channel k=2.0',d.tot)
  fmt(scan('reg',2.5),'reg-channel k=2.5',d.tot)
  console.log(`\n  ✅6w+>base = deploy; else Donchian stays.`)
}

// ─────────────────────────────────────────────────────────────
// v71bt: SLIPPAGE-ROBUSTNESS GATE for the reg-channel breakout that passed
//  v70bt. Its per-trade edge is thin (+0.038R) so it lives/dies on execution
//  cost. Stress Donchian vs reg-channel(k=2.0) under 0/3/6/10 bps/side slippage
//  on every fill. Deploy reg-channel only if its edge survives realistic costs.
function runV71bt() {
  const TK=0.0005, MK=0.0002, NW=6, BAR4=14400000
  const toTf=(a:Bar[],ms:number):Bar[]=>{
    const out:Bar[]=[]; let cur:Bar|null=null; let bucket=-1
  ...
  // ladder returns R + how many fills are taker (entry + trailing/stop legs) vs
  // maker (0.6/1.0R legs). Slippage hits the taker fills (market orders).
  const ladder=(arr:Bar[],j0:number,entry:number,side:'LONG'|'SHORT',slDist:number,atr:number,jEnd:number)=>{
    const dirM=side==='LONG'?1:-1,slPx=entry-slDist*dirM
    const legs=[{r:0.6,frac:1/3},{r:1.0,frac:1/3}]
    const jEndc=Math.min(jEnd,arr.length-1)
  ...
  const regChannel=(arr:Bar[],i:number,W:number)=>{
    ...
    const b=(W*sxy-sx*sy)/den,a=(sy-b*sx)/W
    let s2=0;for(let k=0;k<W;k++){const pred=a+b*k;const r=arr[i-W+k].close-pred;s2+=r*r}
    return {mid:a+b*(W-1),sd:Math.sqrt(s2/W)}}

  // slipBps = adverse slippage per market fill (entry is taker; the trailing/stop
  // exit of the final third is taker; the 0.6/1.0R legs are maker limit = no slip).
  // Entry slip + (1-tpFrac) exit slip, in R units = 2 taker fills' worth roughly.
  const scan=(mode:'donch'|'reg',K:number,slipBps:number)=>{
    const ws:{n:number,sum:number}[]=Array.from({length:NW},()=>({n:0,sum:0}))
    for(const c of Object.keys(d4)){
      if(!CORE40.has(c))continue
      const arr=d4[c];let last=-999
      for(let i=100;i<arr.length-1;i++){
        const price=arr[i].close
        let side:'LONG'|'SHORT'|null=null
        if(mode==='donch'){const prior=arr.slice(i-15,i);let hi=-Infinity,lo=Infinity
          for(const b of prior){if(b.high>hi)hi=b.high;if(b.low<lo)lo=b.low}
          side=price>hi?'LONG':price<lo?'SHORT':null
        }else{const ch=regChannel(arr,i,15);if(!ch||ch.sd<=0)continue
          side=price>ch.mid+K*ch.sd?'LONG':price<ch.mid-K*ch.sd?'SHORT':null}
        if(!side)continue
        const win=arr.slice(Math.max(0,i-99),i+1)
        const adx=calcADX(win.slice(-60));if(adx<=22)continue
        ...
        // taker fills: entry (1 unit) + final-third exit (1-tpFrac). Slippage in R = slip/slPct per fill.
  ...
  console.log(`\n── SLIPPAGE STRESS: Donchian vs reg-channel(k=2.0) ──`)
  console.log(`  slip/side   Donchian totR        reg-channel totR`)
  for(const s of [0,3,6,10]){
    const d=scan('donch',0,s),r=scan('reg',2.0,s)
    const dTag=d.allPos?'✅':'❌',rTag=r.allPos?'✅':'❌'
    console.log(`  ${String(s).padStart(2)} bps      ${d.tot.toFixed(0).padStart(5)} (${(d.avg>=0?'+':'')}${d.avg.toFixed(4)}R) ${dTag}      ${r.tot.toFixed(0).padStart(5)} (${(r.avg>=0?'+':'')}${r.avg.toFixed(4)}R) ${rTag}`)}
  console.log(`\n  live slippage assumption = 3 bps/side. Deploy reg-channel only if it`)
...
// v73bt: DONCHIAN ADAPTIVE — window length scales with volatility regime
// (external-report idea). Prior batches tuned FIXED windows (10/12/15/20/25/40;
// 15 won). Never tested a window that BREATHES with vol. Hypothesis both ways:
// high-vol→WIDER (k>0, filter chop false-breaks) vs high-vol→NARROWER (k<0,
// catch fast moves). W=clamp(round(15·(atrFast/atrRef)^k),8,30). Same ADX gate,
// stop, ladder, 3bps slip. Deploy only if beats fixed-15 AND all-6-windows.
function runV73bt() {
  const TK=0.0005, MK=0.0002, NW=6, BAR4=14400000, SLIP=0.0003
  const toTf=(a:Bar[],ms:number):Bar[]=>{
    const out:Bar[]=[]; let cur:Bar|null=null; let bucket=-1
  ...
  // k=0 → fixed 15 (baseline). k>0 → wider window in high vol; k<0 → narrower.
  const scan=(k:number)=>{
    const ws:{n:number,sum:number}[]=Array.from({length:NW},()=>({n:0,sum:0}))
    let wSum=0,wCnt=0
    for(const c of Object.keys(d4)){
      ...
      for(let i=100;i<arr.length-1;i++){
        const price=arr[i].close
        const atrFast=calcATR(arr.slice(i-20,i+1))
        const atrRef=calcATR(arr.slice(i-100,i+1))
        if(!atrFast||!atrRef)continue
        const W=k===0?15:Math.max(8,Math.min(30,Math.round(15*Math.pow(atrFast/atrRef,k))))
        const prior=arr.slice(i-W,i);let hi=-Infinity,lo=Infinity
        for(const b of prior){if(b.high>hi)hi=b.high;if(b.low<lo)lo=b.low}
        const side:'LONG'|'SHORT'|null=price>hi?'LONG':price<lo?'SHORT':null
        if(!side)continue
        const win=arr.slice(Math.max(0,i-99),i+1)
        ...
        const slipR=(1+(1-res.tpFrac))*SLIP/slPct
        const net=res.r-(TK+res.tpFrac*MK+(1-res.tpFrac)*TK)/slPct-slipR
        const w=ws[winOf(arr[i].t)];w.n++;w.sum+=net;wSum+=W;wCnt++}
    }
    const tot=ws.reduce((a,x)=>({n:a.n+x.n,sum:a.sum+x.sum}),{n:0,sum:0})
    const wA=ws.map(x=>x.n?x.sum/x.n:0)
    return {n:tot.n,avg:tot.sum/tot.n,tot:tot.sum,allPos:wA.every(x=>x>0),avgW:wSum/wCnt,wA}}

  console.log(`\n── DONCHIAN ADAPTIVE: window breathes with vol (atrFast/atrRef)^k ──`)
  console.log(`  k        avgW    n       totR     avgR      all6   windows`)
  const base=scan(0)
  console.log(`  0 (fix)  15.0   ${String(base.n).padStart(5)}   ${base.tot.toFixed(0).padStart(5)}  ${(base.avg>=0?'+':'')}${base.avg.toFixed(4)}   ${base.allPos?'✅':'❌'}    ${base.wA.map(x=>(x>=0?'+':'')+x.toFixed(3)).join(' ')}`)
  for(const k of [-0.5,-0.25,0.25,0.5]){
    const s=scan(k)
    const tag=(s.tot>base.tot&&s.allPos)?'  ← beats fixed-15':''
    console.log(`  ${(k>=0?'+':'')}${k.toFixed(2)}  ${s.avgW.toFixed(1).padStart(5)}   ${String(s.n).padStart(5)}   ${s.tot.toFixed(0).padStart(5)}  ${(s.avg>=0?'+':'')}${s.avg.toFixed(4)}   ${s.allPos?'✅':'❌'}    ${s.wA.map(x=>(x>=0?'+':'')+x.toFixed(3)).join(' ')}${tag}`)
  }
  console.log(`\n  DEPLOY BAR: an adaptive k must beat fixed-15 totR (${base.tot.toFixed(0)}R) AND stay`)
  console.log(`  ✅ all-6-windows. Note fixed-15 is the incumbent; adaptive adds a`)
...
function runV106bt() {
  ...
  const BRK = { dayLossHalt: 0.10, ddHalt: 0.25, lossStreak: 4, streakPauseMs: H }
  // the live v92.0 shape: ROTA holds 50% of the book (25% per side), 1x, ensemble 7/14/28d, no vol damping
  const LIVE: Partial<PF.SimConfig> = { leverage: 1, rotaLbs: [42, 84, 168], rotaBook: 0.25, breakers: BRK }
  const V104: Partial<PF.SimConfig> = { ...LIVE, volTarget: { target: 0.5 } }
  const cands: { tag: string; cfg: Partial<PF.SimConfig> }[] = [
    { tag: 'LIVE v92 (no volT)', cfg: LIVE },
    { tag: 'v104 volT50%', cfg: V104 },
  ]
  for (const base of [{ n: 'noVT', c: LIVE }, { n: 'volT50', c: V104 }]) {
    for (const [hi, lo] of [[70, 30], [75, 25], [80, 20]])
      cands.push({ tag: `${base.n} exhaust rsi14 ${hi}/${lo}`, cfg: { ...base.c, rotaOsc: { mode: 'exhaust', len: 14, hi, lo } } })
    cands.push({ tag: `${base.n} confirm rsi14>50`, cfg: { ...base.c, rotaOsc: { mode: 'confirm', len: 14, hi: 70, lo: 30 } } })
    cands.push({ tag: `${base.n} confirm rsi6>50`, cfg: { ...base.c, rotaOsc: { mode: 'confirm', len: 6, hi: 70, lo: 30 } } })
  }
  console.log(`\n── IN-SAMPLE (selection happens here, and only here) — K2, 12h, 1x, ROTA 50% of the book, breakers on ──`)
  hdr()
  const graded = cands.map(c => { const a = agg(isWins, c.cfg); row(c.tag, a); return { ...c, a } })
  const pick = graded.slice().sort((x, y) =>
    (x.a.ddH - y.a.ddH) || (y.a.net.filter(v => v > 0).length - x.a.net.filter(v => v > 0).length) || (tot(y.a) - tot(x.a)))[0]
  ...
  hdr()
  row(pick.tag, agg(oos, pick.cfg))
  row('LIVE v92 (no volT)', agg(oos, LIVE))
  row('v104 volT50%', agg(oos, V104))
  row(pick.tag + ' @10bps', agg(oos, { ...pick.cfg, slipBps: 10 }))
  console.log(`\n  RSI on completed 4h closes; a rejected name is replaced by the next in rank (pool 3K per side).`)
}
...
function main() {
  ...
  if (Deno.env.get('BT_MODE') === 'v70bt') {
    console.log(`\u2588\u2588\u2588\u2588 V70BT \u2014 regression-channel breakout vs Donchian \u2588\u2588\u2588\u2588`)
    runV70bt()
    return
  }
  if (Deno.env.get('BT_MODE') === 'v71bt') {
    console.log(`\u2588\u2588\u2588\u2588 V71BT \u2014 slippage-robustness gate for reg-channel \u2588\u2588\u2588\u2588`)
    runV71bt()
    return
  }
  ...
  if (Deno.env.get('BT_MODE') === 'v106bt') {
    console.log('████ V106BT — ROTA + RSI agreement (exhaust / confirm), live v92 shape, 36m, 80/20 OOS ████')
    runV106bt()
    return
  }
