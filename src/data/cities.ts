// City coordinates for the /iss/:city landing pages.
//
// Kept in its own module (not inside CityPage) so the homepage footer can list
// the cities without pulling the whole CityPage route chunk into the entry
// bundle, and so api/sitemap.ts derives its URL list from the same source
// instead of a hand-maintained duplicate that silently drifts.

export type City = { name: string; nameEn: string; lat: number; lng: number; desc: string }
