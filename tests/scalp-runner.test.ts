...
const db={from:(table:string)=>{const builder:any={};for(const k of ['select','eq','order','limit'])builder[k]=()=>builder;builder.throwOnError=async()=>({data:table==='bot_trades'?[old]:[]});return builder},rpc:(name:string,args:any)=>{assert.equal(name,'scalp_commit_cycle');request=args;return {throwOnError:async()=>({data:{ok:true}})}}}
...
try{
 ...
 assert.equal(request.p_entries.length,8);assert.equal(request.p_minutes.length,9)
 ...
 assert.ok(request.p_minutes.every((m:any)=>['scout','regime','rota','donch','risk','trader','treasurer','reporter','auditor'].includes(m.who)))
 assert.equal(new Set(request.p_minutes.map((m:any)=>m.who)).size,9)
