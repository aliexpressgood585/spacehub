// ════════════════════════════════════════════════════════════════════════════
// PATH 4: EXTREME CONFLUENCE GATING - COMPREHENSIVE BACKTEST
//
...
async function fetchBars(sym: string, limit = 2000): Promise<Bar[]> {
  try {
...
function scoreVPOCAlignment(
  ...
  // VPOC Alignment (0-25 pts): Price within 0.5-0.8% of Point of Control
  const distPct = Math.abs(price - vpoc) / vpoc
  const aligned = side === 'LONG' ? vpoc < price : vpoc > price

  if (!aligned) return 0
  if (distPct < 0.005) return 25 // Excellent alignment
  if (distPct < 0.008) return 20 // Good alignment
  if (distPct < 0.012) return 15 // Moderate alignment
  if (distPct < 0.018) return 10 // Weak alignment
  return 0
}

function score15mConfirmation(bars5m: Bar[], bars15m: Bar[], side: 'LONG' | 'SHORT'): number {
  // 15m Confirmation (0-20 pts): 15m trend matches 5m direction
  ...
  if (trend15m === expectedTrend && trend5m === expectedTrend) return 20 // Perfect confirm
  if (trend15m === expectedTrend && trend5m !== 'FLAT') return 15 // 15m confirms
  if (trend5m === expectedTrend) return 10 // At least 5m aligns
  if (trend5m !== 'FLAT') return 5 // Some directional bias
  return 0
}

function scoreADXTrending(bars: Bar[]): number {
  // ADX Trending (0-10 pts): ADX > 22
  const adx = calcADX(bars, 14)
  if (adx > 22) return 10 // Strong trend
  if (adx > 18) return 7 // Moderate trend
  if (adx > 14) return 4 // Weak trend
  return 0
}

function scoreVolumeSurge(bars: Bar[], lookback = 20): number {
  // Volume Surge (0-10 pts): Volume > 1.4× 20-day average
  ...
  if (ratio > 1.6) return 10 // Extreme surge
  if (ratio > 1.4) return 8 // Strong surge
  if (ratio > 1.2) return 5 // Moderate surge
  if (ratio > 1.0) return 2 // Slight surge
  return 0
}

function scoreOIDivergence(): number {
  // OI Divergence (0-15 pts): Open Interest favorable
  // Note: This would require live OI data fetch, simulating as favorable in 50% of cases
  // In production: fetch OI data and score based on divergence
  return Math.random() > 0.5 ? 12 : 3
}

function scoreLiquiditySignal(): number {
  // Liquidity Signal (0-15 pts): Liquidation flow favorable
  // Note: This would require live liquidation data, simulating favorability
  // In production: fetch liquidation flow data and score accordingly
  return Math.random() > 0.4 ? 12 : 3
}

function scoreFearGreedIndex(fearGreedValue: number, side: 'LONG' | 'SHORT'): number {
  // Fear/Greed Index (0-5 pts): Extreme sentiment readings
  // fearGreedValue: 0-100 (0=extreme fear, 100=extreme greed)

  if (side === 'LONG') {
    if (fearGreedValue < 25) return 5 // Extreme fear = good for longs
    if (fearGreedValue < 40) return 3
    if (fearGreedValue > 75) return -2 // Greed = bad for longs
    return 1
  } else {
    if (fearGreedValue > 75) return 5 // Extreme greed = good for shorts
    if (fearGreedValue > 60) return 3
    if (fearGreedValue < 25) return -2 // Fear = bad for shorts
    return 1
  }
}
...
async function generatePath4Signal(
  coin: string,
  bars5m: Bar[],
  bars15m: Bar[],
  fearGreedValue: number
): Promise<PathSignal | null> {
  ...
  // Determine entry side based on VPOC alignment and recent trend
  ...
  // Calculate Path 4 scoring components
  ...
  const oiScore = scoreOIDivergence()
  const liqScore = scoreLiquiditySignal()
  ...
  return {
    ...
    adxScore,
    volumeScore,
    oiScore: Math.max(0, oiScore),
    liqScore: Math.max(0, liqScore),
...
function simulateTrade(
  ...
  const slPct = 0.02 // 2% stop loss
  const tpPct = 0.04 // 4% take profit
  const slPrice =
    signal.side === 'LONG'
  ...
  for (let i = startIdx; i < Math.min(startIdx + maxHoldBars, bars.length); i++) {
    ...
    if (signal.side === 'LONG') {
      if (bar.high >= tpPrice) {
        ...
        const pnlPct = (exitPrice - signal.price) / signal.price
        return {
          coin: signal.coin,
          ...
          exitPrice,
          side: 'LONG',
          pnl: 100 * pnlPct, // Assuming $100 per trade
          pnlPct,
      ...
      if (bar.low <= tpPrice) {
        const exitPrice = tpPrice
        const pnlPct = (signal.price - exitPrice) / signal.price
        return {
          coin: signal.coin,
          entryTime: signal.time,
          entryPrice: signal.price,
...
async function runPath4Backtest() {
  ...
  for (const coin of COINS) {
    console.log(`\nFetching ${coin}...`)
    const bars5m = await fetchBars(coin, 2000) // ~7 days of 5m data
    const bars15m = await fetchBars(coin, 666) // ~7 days of 15m data (approx)

    if (!bars5m || bars5m.length < 100) {
      console.log(`  ⚠️  Insufficient data for ${coin}`)
      results[coin] = {
        rawSignals: 0,
        gatedEntries: 0,
        trades: [],
    ...
    console.log(`  ✓ Fetched ${bars5m.length} 5m bars, ${bars15m?.length || 0} 15m bars`)

    const fearGreedValue = 50 // Neutral default - could be fetched
    ...
    for (let i = 100; i < bars5m.length - 10; i++) {
      const signal = await generatePath4Signal(
        coin,
        bars5m.slice(0, i + 1),
      ...
      if (!signal.passed) continue // Gate filter
    ...
    results[coin] = {
      rawSignals: rawSignalCount,
      gatedEntries: gatedSignalCount,
      trades: coinTrades,
      wins,
      losses,
      winRate,
    ...
    console.log(
      `  Raw: ${rawSignalCount} | Gated: ${gatedSignalCount} | Trades: ${coinTrades.length} | WR: ${(winRate * 100).toFixed(1)}% | PF: ${profitFactor.toFixed(2)}`
    )
  }
  ...
  const sorted = Object.entries(results)
    .filter(([_, r]) => r.trades.length >= 3)
    .sort((a, b) => b[1].winRate - a[1].winRate)
    .slice(0, 5)

  for (const [coin, r] of sorted) {
    console.log(
      `│ ${coin.padEnd(6)} ${(r.winRate * 100).toFixed(1).padStart(5)}%  (${r.wins}/${r.trades.length} trades, PF: ${r.profitFactor.toFixed(2)})`
    )
  }
  console.log('└─────────────────────────────────────────────────────────────────────────────┘')

  // Worst performing coins (most filtered)
  console.log('\n┌─ Most Filtered Coins (Lowest Gated Entries) ────────────────────────────┐')
  const filtered = Object.entries(results)
    .sort((a, b) => a[1].gatedEntries - b[1].gatedEntries)
    .slice(0, 5)

  for (const [coin, r] of filtered) {
    const filterRate =
      r.rawSignals > 0
        ? ((1 - r.gatedEntries / r.rawSignals) * 100).toFixed(1)
        : '0.0'
    console.log(
      `│ ${coin.padEnd(6)} ${r.gatedEntries} entries (${filterRate}% filtered from ${r.rawSignals} signals)`
    )
  }
  console.log('└─────────────────────────────────────────────────────────────────────────────┘')
  ...
  const minGating = Math.min(...COINS.map(c => results[c].gatedEntries))
  const maxGating = Math.max(...COINS.map(c => results[c].gatedEntries))
  const avgGating =
    totalGatedEntries / COINS.length
  const coinsAboveWinRate =
    Object.values(results).filter((r: any) => r.winRate > 0.5 && r.trades.length >= 3)
      .length
  ...
  if (totalWinRate >= 0.55 && totalPF >= 1.2 && coinsAboveWinRate >= 12) {
    ...
    console.log(`│ ❌ NOT SUSTAINABLE: Win rate or profit factor too low`)
  }
  console.log('└─────────────────────────────────────────────────────────────────────────────┘')
  ...
  const daysPerMonth = 22 // Trading days
  const projectedTrades = tradesPerDay * daysPerMonth
  const avgPnlPerTrade =
    (totalWinPnl - totalLossPnl) / (totalTrades || 1)
  const monthlyProjPnl = projectedTrades * avgPnlPerTrade

  console.log(
    `│ Projected Trades (20/day × 22 days): ${projectedTrades.toLocaleString()}`
  )
  console.log(`│ Average P&L per Trade: $${avgPnlPerTrade.toFixed(2)}`)
  console.log(
    `│ Projected Monthly P&L: $${monthlyProjPnl.toFixed(0)} (${(monthlyProjPnl / 5000 * 100).toFixed(0)}% on $5k starting capital)`
  )
  console.log('└─────────────────────────────────────────────────────────────────────────────┘')

  console.log(
    '\n✓ Backtest complete. Path 4 Extreme Confluence Gating analysis finished.\n'
  )
}
